...
#define kDefaultBadgeBackgroundColor RED_COLOR
#define kDefaultOverlayColor [UIColor colorWithWhite:1.0f alpha:0]

#define kDefaultBadgeTextFont [UIFont systemFontOfSize:14]
...
#define kBadgeTextSideMargin 5.0f
...
//    CGFloat viewWidth = textWidth + kBadgeTextSideMargin + (kMarginToDrawInside * 2);
    ...
    if (viewHeight / 2 > textWidth) {
        viewWidth = viewHeight;
    }
    else
    {
        viewWidth = textWidth + kBadgeTextSideMargin + (kMarginToDrawInside * 2);
    }
//    CGFloat viewWidth = viewHeight + textWidth - 8.0f;
//    CGFloat viewHeight = 18.0f;
    ...
    switch (self.badgeAlignment) {
        ...
        case JSBadgeViewAlignmentTopRight:
            newFrame.origin.x = superviewWidth - (viewWidth / 2.0f) + 10.0f;
            newFrame.origin.y = -viewHeight / 2.0f + 8.0f;
            break;
...
- (CGSize)sizeOfTextForCurrentSettings
{
    NSDictionary *attr = @{NSFontAttributeName: self.badgeTextFont};
    return [self.badgeText sizeWithAttributes:attr];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (anyTextToDraw)
    {
        ...
        CGContextSaveGState(ctx);
        {
            ...
            NSDictionary *attributes = @{ NSFontAttributeName: self.badgeTextFont,
                                          NSParagraphStyleAttributeName: paragraphStyle,NSForegroundColorAttributeName:kDefaultBadgeTextColor};
